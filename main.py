...
# q = Query("*=>[KNN 2 @embedding $vec AS score]").sort_by("score", asc=True).return_field("score").return_field("content").dialect(2)
# res = r.ft("vector_idx").search(q, query_params={"vec": model.encode(sentence).astype(np.float32).tobytes()})
